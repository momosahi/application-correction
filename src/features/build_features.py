...
def create_variable(data: pd.DataFrame, variable_name: str = "Name"):
    ...
    """
    data["Title"] = data[variable_name].apply(lambda x: x.split(",")[1]).apply(lambda x: x.split()[0])
...
def label_encoder_titanic_column(data: pd.DataFrame, column: str = "Sex") -> pd.DataFrame:
...
def split_train_test_titanic(data: pd.DataFrame, y_index: int = 0, fraction_test: float = 0.2):
